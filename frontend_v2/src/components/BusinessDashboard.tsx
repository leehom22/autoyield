...
export default function BusinessDashboard() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'auto' }}>
        ...
        <div style={{ maxHeight: 260, overflow: 'auto' }}>
          {tab === 'inv' && <table className="data-table"><thead><tr><th>Item</th><th className="text-right">Qty</th><th className="text-right">Cost</th><th className="text-right">Min</th><th className="text-right">Expiry</th><th className="text-right">Days</th><th>Risk</th></tr></thead><tbody>
            {inv.map((i) => { const d = dte(i.expiry_timestamp); const ls = i.qty <= (i.min_stock_level ?? 0); const er = d !== null && d <= 3; return (
              <tr key={i.id}><td className="truncate" style={{ maxWidth: 130 }}>{i.name || i.id}</td><td className="text-right mono">{i.qty}</td><td className="text-right mono">RM{i.unit_cost}</td><td className="text-right mono text-2">{i.min_stock_level ?? '—'}</td><td className="text-right mono text-2" style={{ fontSize: 10 }}>{i.expiry_timestamp ? new Date(i.expiry_timestamp).toLocaleDateString() : '—'}</td><td className="text-right mono" style={{ color: er ? 'var(--red)' : 'var(--text-1)' }}>{d !== null ? `${d}d` : '—'}</td>
          ...
          {tab === 'menu' && <table className="data-table"><thead><tr><th>Name</th><th>Category</th><th className="text-right">Price</th><th className="text-right">Margin</th><th>Status</th></tr></thead><tbody>
            {menu.map((m) => <tr key={m.id}><td>{m.name}</td><td className="text-2">{m.category}</td><td className="text-right mono">RM{m.current_price}</td><td className="text-right mono">{m.margin_percent}%</td><td><span className={`badge ${m.status === 'active' ? 'badge-green' : m.status === 'promo' ? 'badge-cyan' : 'badge-orange'}`}>{m.status}</span></td></tr>)}
          </tbody></table>}
            ...
            {supp.map((s) => <tr key={s.id}><td>{s.name}</td><td className="text-right"><span className={`mono ${s.reliability_score >= 0.8 ? 'text-green' : s.reliability_score >= 0.5 ? 'text-orange' : 'text-red'}`}>{Math.round(s.reliability_score * 100)}%</span></td><td className="text-right mono">{s.avg_lead_time}h</td><td className="text-2 truncate" style={{ maxWidth: 180, fontSize: 10 }}>{typeof s.pricing_tiers === 'object' ? JSON.stringify(s.pricing_tiers) : s.pricing_tiers}</td>
              <td>
                <button
                  ...
                  onClick={() => setContactModal({ id: s.id, name: s.name })}
                  style={{ fontSize: 9, gap: 3 }}
                >
                  <MessageCircle size={10} /> View Contact Logs
                </button>
              </td>
            </tr>)}
          </tbody></table>}
      ...
      {/* ── KDS Queue + Procurement Logs (P1) ── */}
      <div style={{ display: 'flex', flexShrink: 0, borderTop: '1px solid var(--border)', minHeight: 280 }}>
      ...
      {/* ── Knowledge Base (P2) ── */}
      <div style={{ flexShrink: 0, borderTop: '1px solid var(--border)', minHeight: 200 }}>
        <KnowledgeBase />
      </div>

      {/* ── Supplier Contact Logs Modal (P2) ── */}
      {contactModal && (
        <SupplierContactLogsModal
          supplierId={contactModal.id}
          supplierName={contactModal.name}
